...
export default function SubscriptionPage() {
  ...
  const [paying, setPaying] = useState<Interval | null>(null);
  ...
  async function checkout(billingInterval: Interval) {
    setPaying(billingInterval);
    setError(null);
    try {
      const response = await axios.post<{ checkoutUrl: string }>("/api/subscription/checkout", { billingInterval });
  ...
  return (
    <div className="min-h-screen bg-brand-bg">
      ...
      <main className="mx-auto w-full max-w-5xl space-y-6 p-4 sm:p-6 lg:p-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-950">{t("schoolSubscription.title")}</h1>
          ...
          {(data?.plans ?? []).map((plan) => {
            ...
            return (
              <article key={plan.id} className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                <button type="button" disabled={!data?.configured || Boolean(paying)} onClick={() => void checkout(plan.billing_interval)} className="mt-6 w-full rounded-xl bg-gray-950 px-4 py-3 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40">
                  {paying === plan.billing_interval ? t("schoolSubscription.processing") : t("schoolSubscription.renew")}
                </button>
              </article>
            );
          })}
