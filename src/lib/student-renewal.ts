...
export const renewalFields = z.object({
  ...
  cycleFee: z.number().min(0).nullable().optional(),
...
export async function renewStudentSubscription(input: RenewalInput, context: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const changedTerms = !automatic && (
      ...
      (input.cycleFee !== undefined && String(input.cycleFee) !== String(current.cycleFee)));
    if (previousEnd && previousEnd >= today && changedTerms) throw new RenewalError("ACTIVE_TERMS_CHANGE", 409);
    const alreadyRenewed = automatic && previousEnd?.getTime() === end.getTime() && current.enrollment_date && storedDate(current.enrollment_date).getTime() === today.getTime();
    ...
    const cycleFee = automatic || input.cycleFee === undefined ? current.cycleFee : input.cycleFee;
    const interval = automatic || input.billingIntervalDays === undefined ? current.billingIntervalDays : input.billingIntervalDays;
    const settings = cycleFee == null ? await tx.settings.findUnique({
      where: { schoolId: context.schoolId }, select: { dailyStudentFee: true, monthlyStudentFee: true },
    }) : null;
    const effectiveFee = resolveStudentCycleFee(billingCycle, cycleFee, settings);
    if (effectiveFee === null || !effectiveFee.isFinite() || effectiveFee.isNegative()) throw new RenewalError("CYCLE_FEE_REQUIRED");
    ...
    const updated = await tx.student.update({
      ...
      data: {
        ...
        billingCycle, cycleFee, billingIntervalDays: interval,
        ...(!alreadyRenewed && (newPeriod || (previousEnd && end > previousEnd)) ? { paymentStatus: "PENDING" as const } : {}),
        ...(needsReactivation ? {
    ...
    const createdCycles = !alreadyRenewed && from <= end ? await generatePaymentCycles(context.id, tx, from) : 0;
