...
type SubscriptionsData = {
  schools: { id: string; name: string; plan: Plan | null; subscription_status: string; renewal_date: string | null; studentCount: number; daysUntilRenewal: number | null }[];
...
export default function SubscriptionsPage() {
  ...
  const extend = (schoolId: string) => run(async () => { await axios.put(`/api/admin/subscriptions/${schoolId}`, { action: "extend" }); await load(); }, "تعذر تجديد الاشتراك");
  const changePlan = (schoolId: string, plan_id: string) => run(async () => { await axios.put(`/api/admin/subscriptions/${schoolId}`, { action: "change_plan", plan_id }); await load(); }, "تعذر تغيير الخطة");
  ...
  return (
    <div className="mx-auto w-full max-w-[1500px] space-y-8 p-4 sm:p-6 lg:p-8">
      <div><h1 className="text-2xl font-bold text-white">الاشتراكات</h1><p className="mt-1 text-sm text-gray-400">خطتان ثابتتان للمدارس دون أنواع إضافية</p></div>
          ...
          <table className="min-w-[900px] w-full text-sm">
            ...
            <tbody className="divide-y divide-white/5">{data?.schools.map((school) => {
              ...
              return <tr key={school.id} className={days !== null && days < 0 ? "bg-red-500/5" : days !== null && days <= 7 ? "bg-orange-500/5" : ""}>
                ...
                <td className="px-5 py-3"><select value={plans.some((plan) => plan.id === school.plan?.id) ? school.plan?.id : ""} onChange={(event) => void changePlan(school.id, event.target.value)} className="rounded-lg border border-white/10 bg-[#0f0f1a] px-2 py-1 text-xs text-white"><option value="" disabled>{school.plan ? `خطة سابقة: ${school.plan.name}` : "اختاري خطة"}</option>{plans.map((plan) => <option key={plan.id} value={plan.id}>{planLabel(plan)} — {plan.price} ر.س</option>)}</select></td>
                <td className={`px-5 py-3 text-xs font-medium ${STATUS_CLS[school.subscription_status] ?? "text-gray-400"}`}>{school.subscription_status}</td>
                ...
                <td className="px-5 py-3"><button type="button" onClick={() => void extend(school.id)} className="rounded-lg bg-indigo-600/20 px-3 py-1 text-xs text-indigo-300 hover:bg-indigo-600/40">تجديد حسب الخطة</button></td>
              </tr>;
            })}</tbody>
          </table>
        </div>
